...
# Functions for PCA
# ==============================================
#' Bayesian Hyperparameter optimization for PCA
#'
#' Get optimal penalties using bayesian optimization.
#' Score function depends on whether sparsity is assumed or not
#' @param C estimated covariance matrix
...
#' @returns a list object containing the optimal hyperparameters and a summary
#' of the optimization process
bayesian_optimization_PCA <- function(C, p, n,
                                      alpha_x,
                                      order,
                                      low_a,
                                      tol, lr, epochs,
  ...
  scoringFunction <- function(pen_x){
    ...
    if (criterion == "TPO"){
      score <- abs(R)*(1 - p_rate/p * alpha_x[order] + 1e-5)
    } else if (criterion == "BIC"){
      res_full <- train_PCA(C, p,
                            sqrt(min(p, n)),
                            order,
                            0,
                            tol = tol, lr = lr, epochs = epochs,
      ...
      if(order > 1){
        A_full <- cbind(low_a,res_full$best_a)
        A_const <- cbind(low_a,res_train$best_a)
      } else {
        A_full <- res_full$best_a
        A_const <- res_train$best_a
      }

      var_full <- sum(apply(data - data %*% (A_full %*% t(A_full)), 2, var))
      var_constrained <- sum(apply(data - data %*% (A_const %*% t(A_const)), 2, var))

      score <- var_constrained / var_full + p_rate * log(n)/n
  ...
  optObj <- c()
  optObj$scoreSummary <- NA
  best_params <- list(pen_x = max(bounds$pen_x)* sqrt(min(p, n)))
  summary_list <- list()
